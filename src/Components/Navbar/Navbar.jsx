    ...
    return (
        ...
        md:px-20">
            <span className="text-xl font-bold tracking-wide">Portfolio</span>
                <ul className= {`${menu ? "block" : "hidden"} mx-24 -y2 mt-4 font-semibold md:mt-5 bg-black px-2 rounded-xl bg-opacity-30 md:border-none text-center md:bg-transparent md:static md:mx-0 md:flex gap-6`}>
                    <a href="#About">
                ...
                {showMenu ? (
                    <RiMenu2Line size={30} className="md:hidden absolute right-10 top-6 tarnsition-all duration-300" onClick={() =>
                    {openMenu(!menu); setShowmenu(!showMenu);}}/>

                ):<RiCloseLine size={30} className="md:hidden absolute right-10 top-6 tarnsition-all duration-300"/>}
        </nav>
    );
};
